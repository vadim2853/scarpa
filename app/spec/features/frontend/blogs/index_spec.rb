...
describe 'Blogs index', js: true do
  Given!(:april_entry) { create :blog_entry, posted_on: '2015.04.13' }
  Given!(:may_entry) { create :blog_entry, posted_on: '2015.05.13' }

  When { visit_as_adult blogs_path }
  ...
  context 'select May' do
    ...
    And  { expect(find('.article')).to have_link(may_entry.title, href: blog_path(may_entry)) }
  ...
  context 'select April' do
    ...
    Then { expect(find('.left_side')).to have_css('.article', count: 1) }
    And  { expect(find('.article')).to have_link(april_entry.title, href: blog_path(april_entry)) }
